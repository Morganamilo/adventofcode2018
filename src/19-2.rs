...
fn execute(inst: &Instruction, regs: &mut Registers) {
    ...
    "bonr" => reg[inst.c] = reg[inst.a] | reg[inst.b],
    "boni" => reg[inst.c] = reg[inst.a] | inst.b,
...
fn main() {
    ...
    let number = *reg.registers.iter().max().unwrap();
    let total = (1..=number).filter(|i| number % i == 0).sum::<usize>();
    println!("{}", total);
}
